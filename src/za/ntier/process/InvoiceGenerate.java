...
public class InvoiceGenerate extends SvrProcess
{
	...
	/**	The current Invoice	*/
	private MInvoice_New 	m_invoice = null;  // Martin added to update new batch no column
		...
		BigDecimal qtyInvoiced, BigDecimal qtyEntered)
	{
		if (m_invoice == null)
		{
			...
			m_invoice = new MInvoice_New (order, 0, p_DateInvoiced);
			m_invoice.setZZ_Batch_No(1);  // Martin added 24/10/2023
			if (!m_invoice.save())
