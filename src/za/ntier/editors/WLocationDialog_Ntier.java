...
import za.ntier.models.MBPartnerLocation_New;
 ...
 **/
public class WLocationDialog_Ntier extends Window implements EventListener<Event>
{
	/**
	 * generated serial id
	 */
	...
	private MLocation_New   m_location;
	 ...
	 * @param location
	 */
	public WLocationDialog_Ntier(String title, MLocation_New location)
	{
	 ...
	 * @param gridField
	 */
	public WLocationDialog_Ntier(String title, MLocation_New location, GridField gridField) {
		m_GridField  = gridField;
		m_location = location;
		if (m_location == null) {
			m_location = new MLocation_New (Env.getCtx(), 0, null);
		}
		//  Overwrite title 
			...
			if (msg != null) {
				...
				Dialog.error(0, "FillMandatory", Msg.parseTranslation(Env.getCtx(), msg), new Callback<Integer>() {					
					...
					public void onCallback(Integer result) {
						Events.echoEvent("onSaveError", WLocationDialog_Ntier.this, null);
					}
				});
				inOKAction = false;
				return;
	...
	public void dispose()
	{
		...
		{
			m_location = new MLocation_New(m_location.getCtx(), m_location.get_ID(), null);
		}	
		super.dispose();
	}
