...
import za.ntier.models.MLocation_New;
 ...
 * 
 */
public class WLocationEditor_Ntier extends WEditor implements EventListener<Event>, PropertyChangeListener, ContextMenuListener, IZoomableEditor
{
    private static final String[] LISTENER_EVENTS = {Events.ON_CLICK};
    
    private static final CLogger log = CLogger.getCLogger(WLocationEditor_Ntier.class);
    private MLocationLookup_New     m_Location;
    private MLocation_New           m_value;
     ...
     * @param MLocation_New     location model
    **/
    public WLocationEditor_Ntier(String columnName, boolean mandatory, boolean isReadOnly, boolean isUpdateable,
            MLocationLookup_New mLocation)
    {
     ...
     * @param gridField
     */
    public WLocationEditor_Ntier(GridField gridField) {
    ...
    {    
        if ("onClick".equals(event.getName()))
        {
            if (log.isLoggable(Level.CONFIG)) {
				log.config( "onEvent - " + m_value);
			}
            final WLocationDialog_Ntier ld = new WLocationDialog_Ntier(Msg.getMsg(Env.getCtx(), "Location"), m_value, gridField);
            ...
            ld.addEventListener(DialogEvents.ON_WINDOW_CLOSE, new EventListener<Event>() {
				...
				public void onEvent(Event event) throws Exception {
		            ...
		            {
		                ValueChangeEvent vc = new ValueChangeEvent(WLocationEditor_Ntier.this,getColumnName(),null,ii);
		                fireValueChange(vc);
		            }
