...
public class MWFProcess extends X_AD_WF_Process
{
	...
	private boolean startNext (MWFActivity last, MWFActivity[] activities, PO lastPO, String trxName)
	{
		...
		for (int i = 0; i < transitions.length; i++)
		{
			...
			org.compiere.wf.MWFActivity myLast = new org.compiere.wf.MWFActivity(getCtx(),last.getAD_WF_Activity_ID(),get_TrxName()); // Martin
			if (!transitions[i].isValidFor(myLast))
				continue;
